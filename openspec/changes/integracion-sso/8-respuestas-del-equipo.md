...
| 8 | ¿Quién crea cotizaciones? | Dos formas: **anónimo** en la web (resultado por correo) y **cliente registrado** (más campos, pantalla propia). | D7 parcial: cliente y anónimo, confirmados. ❓ Operaciones/admin desde la consola: no lo respondió. Los `@todo D7` de `QuotePolicy` siguen. |
...
al SSO el día que entra el módulo de la consola que lo necesita (y con su policy). ❓ Confirmar
con el equipo que el PDF es el objetivo y no algo ya decidido para implementar ahora.
